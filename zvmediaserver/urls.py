...
urlpatterns = [
    path(
        "books/getsubcategories/<slug:category_slug>/",
        views.get_subcategory_by_category,
        name="get_subcategory_by_category",
    ),
    path("books/getcategories/", views.get_categories, name="get_categories"),
    path(
        "books/category/<slug:book_category_slug>/",
        views.ShowBookCategory.as_view(),
        name="books_category",
    ),
    path(
        "books/subcategory/<slug:book_subcategory_slug>/",
        views.ShowBookSubcategory.as_view(),
        name="books_subcategory",
    ),
    path(
        "books/browser/<slug:book_slug>",
        views.show_detail_book,
        name="browser_detail_book",
    ),
    path(
        "books/initpdf/<slug:book_slug>", views.book_init_as_pdf, name="pdf_init_book"
    ),
    path(
        "books/reader/<slug:book_slug>",
        views.book_reader_as_pdf,
        name="pdf_reader_book",
    ),
    path(
        "books/extradataupdate/<slug:book_slug>",
    ...
    path(
        "books/editbook/<slug:book_slug>", views.UpdateBook.as_view(), name="edit_book"
    ),
    path("books/newcategory/", views.CreateBookCategory.as_view(), name="add_category"),
    path(
        "books/newsubcategory/",
        views.CreateBookSubcategory.as_view(),
        name="add_subcategory",
    ),
    path(
        "books/author/<slug:author_slug>/",
