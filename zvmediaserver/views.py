...
from zvmediaserver.forms import (
    AddBookAuthorForm,
    AddBookCategoryForm,
    AddBookForm,
...
class ShowBooks(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["categories"] = BookCategory.objects.filter(user=self.request.user)
...
def get_categories(request):
    if request.method == "GET":
        categories = BookCategory.objects.filter(user=request.user)
        response = {}
        count = 0
...
def get_activity(request):
    if request.method == "GET":
        ...
        context = {
            "dataset": datasets,
            "labels": labels,
            "authors": BookAuthor.objects.filter(user=request.user),
            "categories": BookCategory.objects.filter(user=request.user),
            "subcategories": BookSubcategory.objects.filter(user=request.user),
