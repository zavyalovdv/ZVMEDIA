...
from booklibrary.modules.services.utils import get_unique_slugify_models
...
class AddBookForm(ModelForm):
    ...
    def clean(self):
        if self.data["author"]:
            self.AuthorModelObject = Author()
            author_is_exists = Author.objects.filter(
            ...
            if not author_is_exists:
                author = Author.objects.create(
                    ...
                    slug=get_unique_slugify_models(
                        self.AuthorModelObject, self.data["author"]
                    ),
                )
                self.cleaned_data["author"] = Author.objects.filter(
    ...
    def clean(self):
        if self.data["author"]:
            author, is_created = Author.objects.get_or_create(
                user=self.request.user,
                name=self.data["author"],
                slug=unique_slugify_models(self.instance, self.data["author"]),
            )
        ...
        self.cleaned_data["author"] = Author.objects.filter(slug=author.slug)
        return self.cleaned_data
